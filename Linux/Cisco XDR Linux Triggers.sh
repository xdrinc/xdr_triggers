...
ndr_trigger() {
  ...
  for target in 75.75.75.75 64.102.6.247; do
    ...
    # Open a persistent socat UDP socket to target's port 53
    exec 3<>/dev/udp/$target/53
    
    for ((i=1; i<=total; i++)); do
      # Send 1200 zero bytes directly to UDP socket
      dd if=/dev/zero bs=1200 count=1 2>/dev/null >&3
